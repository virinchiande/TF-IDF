...
public class Search extends Configured implements Tool {
   ...
   public int run( String[] args) throws  Exception {
      ...
      FileInputFormat.addInputPath(job, args[0]);  // fetching input from the command line 
      ...
      final int NoOfFiles = fs.listStatus(args[0]).length;/* calculating the number of files in the input folder */
      confg.set("NumberOfFiles",String.valueOf(NoOfFiles));
      confg.set("Query", query.toLowerCase());
      Job job2 = Job.getInstance(confg," wordcount2 ");
      ...
      if(query == "")// if the query does not contain any words then return TFIDF job status
    	  return job2.waitForCompletion( true)? 0 : 1;
    	     ...
    	     FileOutputFormat.setOutputPath(job3, new Path(args[1]));  /* path to store search output */
   ...
   public static class Map2 extends Mapper<LongWritable , Text,  Text ,  Text > {
       ...
       private static final Pattern HASHES= Pattern.compile("#####");
        ...
        throws  IOException,  InterruptedException {
         ...
         for(String arrr: SPACE.split(line))// using Space as a delimiter split key value pairs 
         {
        	 if (arrr.isEmpty()) {
                 continue;
            }
         ...
         c=0;
         String oldkey=firstarray[0];
        
         String oldvalue=firstarray[1];
      ...
      public void reduce( Text word,  Iterable<Text> counts,  Context context)
         ...
         idf=Math.log10(1+(n/c)); // calculating IDF value
          ...
          for(int l=0;l<c;l++){   // storing filename and TF value in an array
        	  ...
        	  String newkey=oldkey+"#####"+array[0];
        	  Double value=Double.parseDouble(array[1]);
        	  Double newval=value*idf;
